...
export default function PromptLibrary({ content }) {
  ...
  const pageContent = {
    da: {
      title: "Prompt Library - AI Prompts til Undervisning",
      description: "Samling af testede og effektive AI-prompts til forskellige fag og undervisningssituationer",
      breadcrumbs: [
        { name: 'Hjem', href: '/' },
        { name: 'Prompt Library', href: '/prompt-library' }
      ],
      relatedContent: [
        { title: 'Lektieplaner med AI Integration', href: '/lesson-plans', description: 'Færdige lektieplaner med AI' },
    ...
    en: {
      ...
      relatedContent: [
        { title: 'Lesson Plans with AI Integration', href: '/lesson-plans', description: 'Complete lesson plans with AI' },
...
export async function getStaticProps() {
  try {
    const daContent = fs.readFileSync(path.join(process.cwd(), 'content/da/prompt-library.md'), 'utf8');
    const enContent = fs.readFileSync(path.join(process.cwd(), 'content/en/prompt-library.md'), 'utf8');
    ...
    console.error('Error loading prompt library content:', error);
    return {
      props: {
        content: {
